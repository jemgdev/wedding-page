...
export const cacheWrapper = (cacheObject) => {
  ...
  const has = (input) =>
    cacheObject.match(input).then((res) => {
      ...
      const expTime =
        Date.parse(res.headers.get("Date")) + parseInt(maxAge) * 1000;
...
export const request = (method, path) => {
  ...
  const baseFetch = (input) => {
    ...
    const abstractFetch = () => {
       ...
       */
      const wrapperFetch = () =>
        window.fetch(input, req).then(async (res) => {
          ...
          while (true) {
            ...
            await callbackFunc(
              receivedLength,
              contentLength,
              window.structuredClone ? window.structuredClone(chunks) : chunks
            );
          }
    ...
    return res
      ...
      .then((b) => {
        ...
        link.download = filename
          ? filename
          : `${downName}.${
              downExt ? downExt : b.type.split("/")?.[1] ?? "bin"
            }`;
  ...
  return {
    ...
    send(transform = null) {
      ...
      return baseFetch(new URL(path, document.body.getAttribute("data-url")))
        .then((res) => {
          if (downName && res.ok) {
            return {
              code: res.status,
